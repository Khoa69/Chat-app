...
const Messenger:React.FC = () =>{
  ...
  useEffect(() => {
    conversationService.getAllConversationsById(auth.user._id)
      .then(({data})=>{
  ...
  useEffect(() => {
    ...
    socket.current.on("getUsers", (users:any) => {
      setOnlineUsers(
        users.filter((f :any)=> auth.user._id != f.userId)
      );
    });
  }, [auth]);
    ...
    <st.wrapContainer>
      {
        ...
        <>
          <Menu handleClick={handleClickConversation} conversations={conversations} currentUser={auth.user}/>
          {
            currentChat?
            <MessagesContainer socket={socket} auth={auth} currentChat={currentChat}/>:
            <st.wrapBoxNoText>
