...
function MessagesContainer({ currentChat, auth, socket }: any) {
  ...
  const getMessage = async (page: number, limit: number, initial: boolean) => {
    await messengerService
      .getAllMessages(currentChat._id, limit, page)
      .then(({ data }) => {
  ...
  const onSubmit = (newMessage: any) => {
    ...
    const receiverId = currentChat.members.find(
      (member: any) => member != newMessage.sender,
    );
          ...
          <st.wrapBoxTop onScroll={() => onScroll()} ref={listInnerRef}>
            ...
            {messages.map((message: any, index: number) => (
              <div ref={scrollRef} key={index}>
                <Messages
                  message={message}
                  own={auth.user._id == message.sender}
                />
              </div>
            ))}
          </st.wrapBoxTop>
